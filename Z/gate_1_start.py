...
def refresh_config():
    """Refresh configuration with 3-tier fallback: Central Server → Local YAML → Hardcoded Defaults"""
    global GATE_OPEN_ANGLE, GATE_CLOSED_ANGLE, TARGET_OPEN_TRAVEL
    import requests
    
    # Define hardcoded defaults as the final fallback (with comments preserved)
    HARDCODED_DEFAULTS = {
        "button_debounce_time": 30.0,     # Button debounce time in seconds
        "target_open_travel": -165,       # Degrees from closed to open
        "gate_transition": 0.27,          # seconds to wait when switching power states, anything below .27 won't move the gate well
        "open_speed": 100,                # Motor speeds (0-100%)
        "close_speed": 10,                # Motor speeds (0-100%)
        "open_ramp_time": 1.0,            # Default from config
        "close_ramp_time": 15.0,          # Ramp times in seconds
        "gate_open_wait_time": 5.0,       # Seconds before auto-closing
        "hold_power": 100,                # When gate is open
        "motor_power_limit": 0.85,        # Maximum power limit for the motor (0-1)
        "closed_hold_power": 100,         # Holding power levels (0-100%)
        "default_speed": 40,              # Motor speeds (0-100%)
        "demo_cycles": 1,                 # Number of open/close cycles per demo
    }
    
    # Step 1: Try to get configuration from central server (TIER 1)
    server_config = {}
    try:
        print("🎯 Attempting to load configuration from central server...")
        response = requests.get(f"{CENTRAL_SERVER_URL}/get_config", timeout=5)
        if response.status_code == 200:
            server_config = response.json()
            print(f"✅ Successfully loaded configuration from central server ({len(server_config)} settings)")
        else:
            print(f"⚠️ Central server returned status {response.status_code}")
    except Exception as e:
        print(f"⚠️ Failed to connect to central server: {e}")
    
    # Step 2: Load local YAML config as fallback (TIER 2)
    local_config = {}
    try:
        print("🔄 Loading local YAML configuration...")
        local_config = load_config()  # Your existing function
        if local_config:
            print(f"✅ Local YAML config loaded ({len(local_config)} settings)")
        else:
            print("⚠️ Local YAML config is empty or not found")
    except Exception as e:
        print(f"⚠️ Error loading local YAML config: {e}")
    
    # Step 3: Apply configuration with 3-tier fallback system
    def get_config_value(key, yaml_key=None):
        """Get config value with 3-tier fallback: server → local → default"""
        if yaml_key is None:
            yaml_key = key
        
        # Tier 1: Central server (highest priority)
        if key.upper() in server_config:
            value = server_config[key.upper()]
            print(f"  📡 {key}: {value} (from central server)")
            return value
        
        # Tier 2: Local YAML config
        if yaml_key in local_config:
            value = local_config[yaml_key]
            print(f"  📁 {key}: {value} (from local YAML)")
            return value
        
        # Tier 3: Hardcoded default (lowest priority)
        if yaml_key in HARDCODED_DEFAULTS:
            value = HARDCODED_DEFAULTS[yaml_key]
            print(f"  🛡️ {key}: {value} (hardcoded default)")
            return value
        
        # Should never reach here, but just in case
        print(f"  ❌ {key}: No value found in any tier!")
        return None
    
    print("\n📋 Configuration Resolution:")
    print("-" * 50)
    
    # Update globals with 3-tier fallback (preserving all your original comments)
    globals().update({
        "BUTTON_DEBOUNCE_TIME": get_config_value("BUTTON_DEBOUNCE_TIME", "button_debounce_time"),  # Button debounce time in seconds
        "TARGET_OPEN_TRAVEL": get_config_value("TARGET_OPEN_TRAVEL", "target_open_travel"),        # Degrees from closed to open
        "GATE_TRANSITION": get_config_value("GATE_TRANSITION", "gate_transition"),                 # seconds to wait when switching power states, anything below .27 won't move the gate well
        "OPEN_SPEED": get_config_value("OPEN_SPEED", "open_speed"),                                # Motor speeds (0-100%)
        "CLOSE_SPEED": get_config_value("CLOSE_SPEED", "close_speed"),                             # Motor speeds (0-100%)
        "OPEN_RAMP_TIME": get_config_value("OPEN_RAMP_TIME", "open_ramp_time"),                    # Default from config
        "CLOSE_RAMP_TIME": get_config_value("CLOSE_RAMP_TIME", "close_ramp_time"),                 # Ramp times in seconds
        "GATE_OPEN_WAIT_TIME": get_config_value("GATE_OPEN_WAIT_TIME", "gate_open_wait_time"),     # Seconds before auto-closing
        "HOLD_POWER": get_config_value("HOLD_POWER", "hold_power"),                                # When gate is open
        "MOTOR_POWER_LIMIT": get_config_value("MOTOR_POWER_LIMIT", "motor_power_limit"),           # Maximum power limit for the motor (0-1)
        "CLOSED_HOLD_POWER": get_config_value("CLOSED_HOLD_POWER", "closed_hold_power"),           # Holding power levels (0-100%)
        "DEFAULT_SPEED": get_config_value("DEFAULT_SPEED", "default_speed"),                       # Motor speeds (0-100%)
        "DEMO_CYCLES": get_config_value("DEMO_CYCLES", "demo_cycles"),                             # Number of open/close cycles per demo
    })
    ...
    if 'GATE_CLOSED_ANGLE' in globals() and GATE_CLOSED_ANGLE is not None:
        ...
        print(f"\n🔧 Recalculated GATE_OPEN_ANGLE to {GATE_OPEN_ANGLE}° (GATE_CLOSED_ANGLE {GATE_CLOSED_ANGLE}° + TARGET_OPEN_TRAVEL {TARGET_OPEN_TRAVEL}°)")
    
    print("-" * 50)
    print("✅ Configuration refresh complete!")
    
    # Summary of what was used
    server_count = len([k for k in server_config.keys() if k.upper() in [
        'BUTTON_DEBOUNCE_TIME', 'TARGET_OPEN_TRAVEL', 'GATE_TRANSITION', 'OPEN_SPEED', 
        'CLOSE_SPEED', 'OPEN_RAMP_TIME', 'CLOSE_RAMP_TIME', 'GATE_OPEN_WAIT_TIME', 
        'HOLD_POWER', 'MOTOR_POWER_LIMIT', 'CLOSED_HOLD_POWER', 'DEFAULT_SPEED', 'DEMO_CYCLES'
    ]])
    yaml_count = len([k for k in local_config.keys() if k in [
        'button_debounce_time', 'target_open_travel', 'gate_transition', 'open_speed', 
        'close_speed', 'open_ramp_time', 'close_ramp_time', 'gate_open_wait_time', 
        'hold_power', 'motor_power_limit', 'closed_hold_power', 'default_speed', 'demo_cycles'
    ]])
    
    print(f"📊 Config source summary: {server_count} from server, {yaml_count} from YAML, {13-(server_count+yaml_count)} from defaults")
...
def race_action_open_gate():
    #Stage 4: Racing
    """Open the gate when race officially starts"""
    global global_motor, current_race_number, formatted_race
    ...
    try:
        ...
        # NOW open the gate and capture the EXACT moment it opens
        if global_motor and is_motor_connected(global_motor):
            
            # 🎯 CAPTURE PRECISE GATE OPENING TIME
            gate_opening_time = time.time()  # High precision timestamp
            
            if open_gate(global_motor):
                logger.info("Gate opened successfully for race start")
                
                # 📡 SEND GATE OPENING TIME TO CENTRAL SERVER
                send_gate_opening_time(gate_opening_time)
...
def send_gate_opening_time(opening_time):
    """Send precise gate opening time to central server"""
    if not sio.connected:
        logger.warning("Cannot send gate opening time - not connected to server")
        return
    
    try:
        # Format timestamp with millisecond precision
        formatted_timestamp = format_time_with_ms()
        ...
        data = {
            ...
            'timestamp': opening_time,  # High precision float
            'formatted_time': formatted_timestamp,  # Human readable
            'milliseconds': int((opening_time % 1) * 1000)  # Exact milliseconds
        }
        
        sio.emit('gate_timing_event', data)
        logger.info(f"Gate opening time sent to server: {formatted_timestamp} ({opening_time})")
        
    except Exception as e:
        logger.error(f"Error sending gate opening time: {e}")
...
def handle_config_update(data):
    """Receive real-time config updates from central server"""
    print("🔄 Real-time config update received from central server")
    print(f"Updated config keys: {list(data.keys())}")
    
    # Refresh config will now get the updated values from server first
    refresh_config()
    
    # Optional: Show which specific values changed
    for key, value in data.items():
        print(f"  📡 {key}: {value} (updated from server)")
    
    print("✅ Configuration updated and applied!")
