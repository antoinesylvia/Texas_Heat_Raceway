...
TIMEOUT_DURATION = getattr(config, 'TIMEOUT_DURATION', 15)
CRASH_WARNING_TIME = getattr(config, 'CRASH_WARNING_TIME', 10)
...
def request_checkpoint_times_for_race(race_number):
    """Request checkpoint times from central server for completed race"""
    if not sio.connected:
        logger.warning("Cannot request checkpoint times - not connected to server")
        return None
        
    try:
        # Request checkpoint data
        sio.emit('get_checkpoint_times', {
            'race_number': race_number,
            'formatted_race': formatted_race,
            'gate_type': 'finish_gate'
        })
        
        logger.info(f"Requested checkpoint times for race {race_number}")
        # Response will be handled in on_checkpoint_times_response
        return latest_checkpoint_times  # Return cached data if available
        
    except Exception as e:
        logger.error(f"Error requesting checkpoint times: {e}")
        return None

def request_track_record_from_server():
    """Request the current track record from the central server"""
    demo_mode = getattr(config, 'DEMO_MODE', False) or '--demo_mode' in sys.argv
    
    if not demo_mode and sio.connected:
        try:
        ...
        logger.info("Cannot request track record from server (demo mode or not connected)")
        return False
...
# Checkpoint gate integration variables
checkpoint_gate_ready = False  # Track if checkpoint gate is available
latest_checkpoint_times = None  # Store latest checkpoint data
...
def initialize_display():
    ...
    try:
        ...
        if not getattr(config, 'DEMO_MODE', False) and getattr(config, 'CENTRAL_SERVER_URL', None):
...
def display_on_screen(race_results_list, current_formatted_race_id, num_lanes_to_display=6, checkpoint_times=None):
    ...
    if current_formatted_race_id:
        try:
            ...
            if race_status == "Finished" and demo_mode:
    ...
    for i in range(num_lanes_to_display):
        ...
        # Checkpoint time display (small text under gap timing)
        if checkpoint_times and lane_number in checkpoint_times:
            checkpoint_time = checkpoint_times[lane_number]
            checkpoint_font = pygame.font.Font("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 14)  # Smaller font
            
            if checkpoint_time is not None and not is_any_dnf:
                checkpoint_text = f"Checkpoint: {checkpoint_time:.3f}s"
                checkpoint_color = (150, 150, 255)  # Light blue
            elif is_any_dnf:
                checkpoint_text = "Checkpoint: DNF"
                checkpoint_color = (255, 0, 0)  # Red for DNF
            else:
                checkpoint_text = "Checkpoint: --"
                checkpoint_color = (128, 128, 128)  # Gray
                
            checkpoint_surface = checkpoint_font.render(checkpoint_text, True, checkpoint_color)
            checkpoint_rect = checkpoint_surface.get_rect(center=(x + column_width // 2, y + 5))
            screen.blit(checkpoint_surface, checkpoint_rect)
            y += checkpoint_surface.get_height() + row_spacing
...
def race_action_inbound_to_finish_gate():
    ...
    # Note: Sensor calibration is now handled in start_countdown_sequence_finish_gate()
    # during the countdown phase for better timing
...
def finish_race_action():
    ...
    for idx, ft in enumerate(finish_times):
        if ft is not None and start_time is not None:
            ...
            if elapsed > 0:
                # Check if this is a timeout DNF (at or very close to timeout duration)
                if abs(elapsed - TIMEOUT_DURATION) < 0.01:  # Within 10ms of timeout
                    # This is a timeout DNF
                    timeout_entries.append({'lane': idx + 1, 'elapsed': elapsed})
                    logger.info(f"Lane {idx + 1} TIMEOUT DNF at {elapsed:.3f}s (timeout duration: {TIMEOUT_DURATION}s)")
                else:
    ...
    # Request checkpoint data if checkpoint gate was ready
    checkpoint_times = None
    if checkpoint_gate_ready and not demo_mode:
        checkpoint_times = request_checkpoint_times_for_race(current_race_number)
        logger.info(f"Retrieved checkpoint times for race {formatted_race}: {checkpoint_times}")
    
    # Call extracted function for placement display
    display_placement_results(results_list, winning_lane_num, checkpoint_times)
    
    # Continue with demo/online mode handling...
    if demo_mode:
        ...
        # Call extracted function for race finished display
        display_race_finished(results_list, winning_lane_num, checkpoint_times)
...
def check_finish_conditions(num_lanes_active=6):
        ...
        if lane_num in light_sensors and finish_times[lane_num - 1] is None: # If not already finished
            ...
            try:
                ...
                if sensor_obj.is_triggered(current_lux):
                    # Capture precise finish time
                    precise_finish_time = current_event_time  # This is perf_counter time
                    finish_times[lane_num - 1] = precise_finish_time
                    
                    # Convert perf_counter to epoch time for server
                    epoch_finish_time = time.time() - (time.perf_counter() - precise_finish_time)
                    ...
                    # Send precise timing event to server
                    send_car_finish_time(lane_num, epoch_finish_time, epoch_race_start)
    ...
    # Enhanced crash detection with warnings
    for lane_idx in range(num_lanes_active): # 0 to num_lanes_active-1
        ...
        if finish_times[lane_idx] is None: # If this lane hasn't recorded a finish time
            elapsed = current_event_time - start_time
            
            if elapsed > TIMEOUT_DURATION:
                # CRASH/TIMEOUT - mark as DNF
                finish_times[lane_idx] = start_time + TIMEOUT_DURATION
                logger.warning(f"🚨 Lane {lane_num_for_log} CRASHED/TIMEOUT after {elapsed:.1f}s - marked as DNF")
                print(f"🚨 LANE {lane_num_for_log}: CRASH DETECTED - Car did not finish!")
                
                # Add DNF to current_results
                dnf_elapsed = TIMEOUT_DURATION
                dnf_speed = None
                current_results[lane_num_for_log] = (lane_num_for_log, None, dnf_elapsed, dnf_speed)
                
            elif elapsed > CRASH_WARNING_TIME:
                # WARNING - car is taking too long
                logger.warning(f"⚠️ Lane {lane_num_for_log} WARNING: {elapsed:.1f}s elapsed, may have crashed")
                print(f"⚠️ LANE {lane_num_for_log}: Slow finish - {elapsed:.1f}s elapsed")
                all_lanes_accounted_for = False # Still waiting for this lane
            else:
    ...
    if all_lanes_accounted_for or num_finished_lanes == num_lanes_active :
        ...
        finish_race_action()
    elif (current_event_time - start_time) > (TIMEOUT_DURATION + 2) and race_in_progress:
        ...
        for lane_idx in range(num_lanes_active):
            if finish_times[lane_idx] is None or finish_times[lane_idx] == float('inf'):
                finish_times[lane_idx] = start_time + TIMEOUT_DURATION  # Set to actual timeout moment
                logger.info(f"Lane {lane_idx + 1} DNF (Timeout). Time set to {TIMEOUT_DURATION:.3f}s.")
        finish_race_action()

def start_countdown_sequence_finish_gate():
    """Prepare finish gate for incoming race during countdown"""
    logger.info("Countdown sequence started - preparing for race start")
    
    # Final sensor preparation/calibration before race
    ...
    if screen and font:
        screen.fill((255, 165, 0))  # Orange background for countdown
        try:
            text_surface = font.render("COUNTDOWN - RACE STARTING", True, (255, 255, 255))
            text_rect = text_surface.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
            screen.blit(text_surface, text_rect)
            pygame.display.flip()
        except Exception as e:
            logger.error(f"Error displaying countdown message: {e}")
    
    logger.info("Finish gate ready for race start")
    return calibration_all_ok

def display_placement_results(results_list=None, winning_lane_num=None, checkpoint_times=None):
    """Display placement results while race is in placement phase"""
    logger.info("Race in placement phase - showing current results")
    
    # Use provided results or fallback to stored results
    if results_list is None and hasattr(main_loop, 'last_race_results'):
        results_list = main_loop.last_race_results
    
    if results_list:
        # Display results on screen (MUST happen before checking demo mode)
        display_on_screen(results_list, formatted_race, num_lanes_to_display=len(finish_times), checkpoint_times=checkpoint_times)
...
def display_race_finished(results_list=None, winning_lane_num=None, checkpoint_times=None):
    """Display final race completion state"""
    logger.info("Race officially finished - displaying final state")
    
    # Use provided results or fallback to stored results
    if results_list is None and hasattr(main_loop, 'last_race_results'):
        results_list = main_loop.last_race_results
    
    # Find winning lane if not provided
    if winning_lane_num is None and results_list:
        for result in results_list:
            if result[1] == 1:  # Place 1
                winning_lane_num = result[0]
                break
...
def intermission_mode():
    print("Entering intermission mode. Press 'R' to reset the gate.")
...
def connect():
    ...
    sio.emit('register_gate', {'gate_type': 'finish_gate', 'gate_id': getattr(config, 'GATE_ID', GATE_ID)})
...
def connect_error(data):
    ...
    # Implement retry logic or fallback to demo mode if desired
...
@sio.on('checkpoint_times_response')
def on_checkpoint_times_response(data):
    """Handle checkpoint times response from server"""
    global latest_checkpoint_times
    
    race_number = data.get('race_number')
    checkpoint_times = data.get('checkpoint_times', {})
    
    # Store for use in display
    latest_checkpoint_times = checkpoint_times
    
    logger.info(f"Received checkpoint times for race {race_number}: {checkpoint_times}")
...
def on_race_command(data):
    ...
    if command == 'start_race':
        ...
        race_action_inbound_to_finish_gate()
...
def on_race_state_update(data):
    ...
    if current_status == 'Initialization':
        ...
        # Check what gates are ready
        ready_gates = data.get('ready_gates', [])
        global checkpoint_gate_ready
        checkpoint_gate_ready = 'checkpoint_gate' in ready_gates
        
        if checkpoint_gate_ready:
            logger.info("Checkpoint gate is ready - checkpoint times will be available")
        else:
            logger.info("Checkpoint gate not ready - race will proceed without checkpoint data")
        ...
        start_countdown_sequence_finish_gate()
        ...
        display_placement_results(checkpoint_times=latest_checkpoint_times)
        ...
        display_race_finished(checkpoint_times=latest_checkpoint_times)
...
def send_race_state_to_server(status_str, winning_lane=None, results=None):
    ...
    data = {
        'gate_id': getattr(config, 'GATE_ID', GATE_ID),
        'gate_type': 'finish_gate',
        'status': status_str,
        'current_time_epoch': current_epoch_time,
        'formatted_time': datetime.fromtimestamp(current_epoch_time).strftime('%H:%M:%S.%f')[:-3],  # ← Add this
        'milliseconds': int((current_epoch_time % 1) * 1000),  # ← Add this
        'race_start_time_perf': start_time,
...
def send_car_finish_time(lane, finish_time, race_start_time):
    """Send precise car finish time to central server"""
    if not sio.connected:
        logger.warning("Cannot send car finish time - not connected to server")
        return
    
    try:
        # Calculate precise elapsed time from race start
        elapsed_time = finish_time - race_start_time
        
        # Format timestamp with millisecond precision like start gate
        formatted_timestamp = datetime.fromtimestamp(finish_time).strftime('%H:%M:%S.%f')[:-3]
        
        data = {
            'race_id': current_race_number,
            'formatted_race': formatted_race,
            'gate_type': 'finish_gate',
            'gate_id': GATE_ID,
            'event_type': 'car_finished',
            'lane': lane,
            'timestamp': finish_time,        # ← High precision float (perf_counter converted to epoch)
            'formatted_time': formatted_timestamp,  # ← Human readable with milliseconds
            'milliseconds': int((finish_time % 1) * 1000),  # ← Exact milliseconds
            'elapsed_time': elapsed_time,    # ← Race time for this car
            'elapsed_milliseconds': int((elapsed_time % 1) * 1000)  # ← Race time milliseconds
        }
        
        sio.emit('gate_timing_event', data)  # ← Use same event type as start gate
        logger.info(f"Car finish time sent: Lane {lane} at {formatted_timestamp} ({elapsed_time:.4f}s)")
        
    except Exception as e:
        logger.error(f"Error sending car finish time: {e}")
...
def run_demo(num_lanes=6):
    """
    Demo mode that runs finish gate testing locally without server connection.
    Allows local race testing with keyboard controls.
    """
    global running_main_loop, formatted_race, current_race_number, race_in_progress
    
    print("\n" + "=" * 60)
    print("FINISH GATE DEMO MODE")
    print("=" * 60)
    print("Local testing mode for the finish gate:")
    print("• SPACE key: Start a new local race")
    print("• R key: Reset gate and start new race")
    ...
    status_send_interval = 30  # Send component status every 30 seconds (if connected)
    display_update_interval = 0.1  # Update screen 10 times per second for live timing
    
    logger.info(f"Demo mode started. Monitoring {num_lanes} lanes locally.")
    ...
    try:
        while running_main_loop:
            ...
            # Handle demo-specific keyboard events
            if screen:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        logger.info("Demo: Pygame QUIT event received. Exiting demo.")
                        running_main_loop = False
                        
                    elif event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_ESCAPE:
                            logger.info("Demo: ESCAPE key pressed. Exiting demo.")
                            running_main_loop = False
                            
                        elif event.key == pygame.K_SPACE:
                            if not race_in_progress:
                                logger.info("Demo: SPACE key - Starting new local race")
                                # Create demo race ID
                                formatted_race = f"Demo_Race_{current_race_number}_{int(time.time())}"
                                print(f"\n🏁 DEMO RACE {current_race_number} STARTING")
                                print(f"Race ID: {formatted_race}")
                                race_action_inbound_to_finish_gate()
                            else:
                                logger.info("Demo: SPACE key - Race already in progress")
                                print("Race already in progress! Wait for it to finish or press R to reset.")
                                
                        elif event.key == pygame.K_r:
                            logger.info("Demo: R key - Resetting gate and starting new race")
                            if race_in_progress:
                                print(f"\n🔄 DEMO RACE {current_race_number} RESET")
                            else:
                                print(f"\n🔄 GATE RESET - READY FOR RACE {current_race_number + 1}")
                            
                            reset_gate_state()
                            current_race_number += 1
                            formatted_race = f"Demo_Race_{current_race_number}_{int(time.time())}"
                            print(f"Next Race ID: {formatted_race}")
                            race_action_inbound_to_finish_gate()

            # Core race logic: check for finishes if a race is active
            if race_in_progress:
                check_finish_conditions(num_lanes_active=num_lanes)

            # Update display at regular intervals (live timing during race)
            if screen and font and (current_loop_time - last_display_update_time >= display_update_interval):
                ...
                if race_in_progress and start_time:
                    ...
                    display_on_screen(live_results, formatted_race, num_lanes_to_display=num_lanes)
                    ...
                    if hasattr(main_loop, 'last_race_results') and main_loop.last_race_results:
                        display_on_screen(main_loop.last_race_results, formatted_race, num_lanes_to_display=num_lanes)

            # Send status periodically (if server is connected - demo can still connect)
        ...
        logger.info("Demo: KeyboardInterrupt detected. Exiting demo mode.")
        running_main_loop = False
        
    except Exception as e:
        logger.error(f"Demo: Unhandled error in demo loop: {e}", exc_info=True)
        ...
        print("=" * 60)
        if race_in_progress:
            print("Last race was still in progress when demo ended.")
        else:
            print(f"Completed {current_race_number} demo races successfully.")
        print("• All finish gate systems tested")
        print("• Local race timing verified")
        print("• Display and sensor integration confirmed")
        print("=" * 60)
        
        logger.info("Demo mode terminated.")
        return True

def main_loop(num_lanes_in_use=6):
    """Main event loop for the finish gate system - SERVER MODE ONLY."""
    ...
    logger.info(f"Finish Gate main loop started in SERVER MODE. Monitoring {num_lanes_in_use} lanes.")
    ...
    while running_main_loop:
        try:
            ...
            # Handle basic Pygame events (quit only - no manual race controls in server mode)
            if screen:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_ESCAPE:
                            ...
                            # In server mode, all other keys are ignored
                            logger.info(f"Server mode: Key {event.key} pressed but ignored. Waiting for server commands.")
            ...
            if screen and font and (current_loop_time - last_display_update_time >= display_update_interval):
                ...
                if race_in_progress and start_time:
                    ...
                    # Race finished - continue showing final results
                    if hasattr(main_loop, 'last_race_results') and main_loop.last_race_results:
    ...
    logger.info("Server mode main loop terminated.")
...
def cleanup_resources():
    ...
    if sio.connected:
        try:
            send_race_state_to_server('Offline') # Inform server gate is going demo
            sio.disconnect()
...
if __name__ == "__main__":
    ...
    NUMBER_OF_LANES = 6
    logger.info(f"Finish Gate System starting for all {NUMBER_OF_LANES} lanes.")
    ...
    # Check for demo mode from CLI argument or config
    demo_mode = args.demo_mode or getattr(config, 'DEMO_MODE', False)

    if demo_mode:
        logger.info("MAIN: Demo mode enabled - skipping server connection")
        print("\n🎯 STARTING FINISH GATE DEMO MODE")
        print("Demo mode allows local testing without server connection")
        print(f"Monitoring all {NUMBER_OF_LANES} lanes")
        
        try:
            if 'run_demo' in globals() and callable(run_demo):
                logger.info("MAIN: Starting demo mode...")
                run_demo(num_lanes=NUMBER_OF_LANES)
            else:
                logger.critical("MAIN: run_demo function is not defined. Cannot start demo.")
                sys.exit(1)
        except Exception as e_demo:
            logger.critical(f"MAIN: Error during demo mode: {e_demo}", exc_info=True)
        finally:
            logger.info("MAIN: Demo mode completed, initiating cleanup...")
            ...
            logger.info("MAIN: Demo application has shut down.")
            sys.exit(0)

    else:
        # Connect to Socket.IO server (server mode)
        try:
            ...
            logger.error(f"MAIN: Socket.IO connection failed: {e}. Consider using --demo_mode for local testing.")
            sys.exit(1)
        except NameError:
            ...
            logger.error(f"MAIN: An unexpected error occurred during Socket.IO connection: {e_sio_connect}. Consider using --demo_mode for local testing.")
            sys.exit(1)

        # Start the main server mode application loop
        try:
            if 'main_loop' in globals() and callable(main_loop):
                logger.info("MAIN: Starting server mode main loop...")
            ...
            logger.critical(f"MAIN: Error during server mode: {e_main_loop_start}", exc_info=True)
        finally:
            ...
            if 'cleanup_resources' in globals() and callable(cleanup_resources):
                ...
                logger.warning("MAIN: cleanup_resources function not defined. Skipping cleanup.")
            logger.info("MAIN: Server mode application has shut down.")
            sys.exit(0)