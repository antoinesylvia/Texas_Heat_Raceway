                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_ESCAPE:
                            ...
                            # Only allow starting a local test race if in demo mode
                            if not race_in_progress and demo_mode:
                                logger.info("SPACE key: Initiating local race start.")
                                # Create a local race ID
                                formatted_race = f"Test_{int(time.time())}"
                                race_action_inbound_to_finish_gate()
                            elif not race_in_progress and not demo_mode:
                                logger.info("SPACE key: Cannot start local race in online mode. Wait for server provided race state.")
                                ...
                                if screen and font:
                                    ...
                                    submessage = "Switch to demo mode or wait for server commands"
...
if __name__ == "__main__":
    ...
    # Connect to Socket.IO server (unless in demo mode)
